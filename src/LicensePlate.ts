export default class LicensePlate {
	 ...
	 * @type {number}
	 */
	private static matchedSidecode: number;
	...
	public static make(licensePlate: string): string {
		...
		this.matchedSidecode = this.getSidecode();
	...
	private static getSidecode(): number {
		for (let index = 0; index < this.sidecodes.length; index++) {
			if (this.licensePlate.match(this.sidecodes[index])) {
				return index + 1;
			}
		}
